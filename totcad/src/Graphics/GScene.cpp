...
void GScene::addEntity(const std::shared_ptr<GEntity> &entity,
...
QColor GScene::entityColor(const GEntity &entity, EntityID selectionId) const
{
    if (m_annotationDocument) {
...
QPen GScene::entityPen(const GEntity &entity, EntityID selectionId) const
{
...
void GScene::refreshStyles()
{
    rebuild();
}

QRectF GScene::instanceBounds(const QString &instanceId) const
{
    if (!m_annotationDocument)
        return {};
...
void GScene::refreshInstanceBoxes()
{
...
QVector<EntityID> GScene::selectedEntityIds() const
{
    QSet<EntityID> ids;
